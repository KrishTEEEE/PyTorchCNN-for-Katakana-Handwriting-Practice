...
The database has custom file formats and needs to be unpacked using the organization's provided Python Package, I then wrote a Python program that automates the process of reformatting the unpacked data into a PyTorch-compatible structure. The images are split into train, validation and test sets, then processed before feeding into the custom CNN for training. Dropout, data augmentation and early stopping was also implemented to prevent overfitting. Since I am using a Mac, I used Apple's Metal Performance Shaders, its GPU framework (similar to using CUDA) for acceleration.

The biggest challenge in this project was finding good data for training, the image sets from Kaggle all had too little data and causes severe overfitting even with data augmentation. If you are interested in the effect of sample number on the performance of a model, have a look at the MLP 
## 📋 Table of Contents
- [Features](#-features)
- [Demo](#-demo)
